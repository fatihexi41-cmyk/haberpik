...
export default function Home() {
  ...
  // KANKA: Kategoriler listesi (Menü ve Slider sekmeleri için)
  const kategorilerListesi = ["SON DAKİKA", "GÜNDEM", "SİYASET", "SPOR", "EKONOMİ", "TEKNOLOJİ", "DÜNYA", "ASAYİŞ", "MAGAZİN"];
  ...
  useEffect(() => {
    ...
    const fetchPiyasa = async () => {
      try {
        ...
        setPiyasa({ DOLAR: tryRate.toFixed(2), EURO: (tryRate * 1.08).toFixed(2), ALTIN: (tryRate * 75).toFixed(0) });
  ...
  const getGazeteLink = (ad: string) => {
    const list: any = { 'Hürriyet': 'hurriyet', 'Sözcü': 'sozcu', 'Sabah': 'sabah', 'Milliyet': 'milliyet', 'Türkiye': 'turkiye', 'Akşam': 'aksam', 'Yeni Şafak': 'yenisafak', 'Karar': 'karar', 'Korkusuz': 'korkusuz' };
    return list[ad] || 'hurriyet';
  };
  ...
  return (
    <div className="bg-[#e6e6e6] text-[#111] font-sans selection:bg-red-600 selection:text-white antialiased">
      ...
      {/* KANKA: AÇILIR MENÜ (Geri geldi, jilet gibi oldu) */}
      {menuAcik && (
        <div className="fixed inset-0 z-[999] bg-[#1a1a1a] p-10 flex flex-col items-center justify-center animate-in fade-in slide-in-from-top duration-300">
          <button onClick={() => setMenuAcik(false)} className="absolute top-10 right-10 text-white hover:text-red-600 transition-colors"><FaIcons.FaTimes size={40}/></button>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-8 text-center">
            {kategorilerListesi.map(m => (
              <Link key={m} href={`/kategori/${m.toLowerCase().replace(" ", "-")}`} onClick={() => setMenuAcik(false)} className="text-white text-3xl font-black italic uppercase hover:text-red-600 tracking-tighter transition-all">{m}</Link>
            ))}
          </div>
          <div className="mt-20 text-red-600 text-5xl font-black italic tracking-tighter">HABERPİK</div>
        </div>
      )}
          ...
          <div className="hidden lg:flex items-center gap-4 font-black text-[10px] text-gray-600 uppercase italic">
            ...
            <div className="relative group" onMouseEnter={() => setNamazVaktiHover(true)} onMouseLeave={() => setNamazVaktiHover(false)}>
              ...
              {namazVaktiHover && namazVakitleri && (
                <div className="absolute top-full left-0 w-48 bg-white shadow-2xl border border-gray-200 z-[999] p-3 mt-1">
      ...
      {/* 2. NAVBAR (Linkler jilet gibi düzeltildi kanka) */}
      <nav className="bg-[#1a1a1a] sticky top-0 z-[800] shadow-sm font-black italic">
        <div className="max-w-[1150px] mx-auto flex items-center justify-between text-[12px] uppercase tracking-tighter text-white">
          <div className="flex items-center overflow-x-auto no-scrollbar">
            ...
            {kategorilerListesi.slice(0, 7).map(m => (
              <Link key={m} href={`/kategori/${m.toLowerCase().replace(" ", "-")}`} className="px-3 py-3 hover:bg-white/10 border-r border-white/5 whitespace-nowrap">{m}</Link>
            ))}
          </div>
          <div className="flex items-center">
            <button className="p-3 hover:bg-white/10"><FaIcons.FaSearch size={16}/></button>
            <button onClick={() => setMenuAcik(!menuAcik)} className="px-4 py-3 bg-red-600 flex items-center gap-2 whitespace-nowrap uppercase">
      ...
      <main className="max-w-[1150px] mx-auto px-1 py-1">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-1">
              <div className="lg:col-span-3 space-y-1">
                 {getKat("SPOR", 6).map((h, i) => (
                    <Link href={`/haber/${h.id}`} key={h.id} className="flex items-center gap-2 bg-gray-50 border p-1 hover:bg-green-50 h-[52px]">
        ...
        {/* KANKA: EKONOMİPİK (Resimli grid'e dönüştürüldü) */}
        <section className="mt-4 grid grid-cols-1 lg:grid-cols-12 gap-1 mb-4">
           <div className="lg:col-span-9 bg-white p-3 border-t-2 border-blue-900 shadow-sm">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-1">
                 {getKat("EKONOMİ", 8).map(h => (
                    <Link href={`/haber/${h.id}`} key={h.id} className="relative h-32 group overflow-hidden border">
                       <img src={h.resim} className="w-full h-full object-cover group-hover:scale-110 transition-transform" />
                       <div className="absolute inset-0 bg-black/40 p-2 flex items-end">
                          <h4 className="text-white text-[9px] font-black uppercase italic leading-tight line-clamp-3 tracking-tighter group-hover:text-blue-400">{h.baslik}</h4>
                       </div>
                    </Link>
                 ))}
              </div>
           ...
           <div className="lg:col-span-3 space-y-1">
              <div className="bg-blue-900 text-white p-1.5 text-center font-black italic uppercase text-xs tracking-tighter">PİYASA EKRANI</div>
              {piyasa ? [
                { n: "DOLAR", v: piyasa.DOLAR, i: <FaIcons.FaDollarSign size={10}/> },
                { n: "EURO", v: piyasa.EURO, i: <FaIcons.FaEuroSign size={10}/> },
                { n: "ALTIN", v: piyasa.ALTIN, i: <FaIcons.FaGem size={10}/> },
                { n: "BIST", v: "12.660", i: <FaIcons.FaChartBar size={10}/> }
              ].map(p => (
                <div key={p.n} className="bg-white p-1.5 flex justify-between items-center border-l-4 border-blue-900 shadow-sm font-black text-[10px] uppercase italic hover:bg-blue-50 cursor-pointer">
                   <div className="flex items-center gap-2"><span className="text-blue-900">{p.i}</span><span>{p.n}</span></div>
                   <div className="text-green-600">{p.v} TL</div>
                </div>
        ...
        <section className="mt-4 bg-[#1a1a1a] p-3 rounded-sm overflow-hidden shadow-xl">
           ...
           <Swiper modules={[Autoplay, Navigation]} slidesPerView={3} breakpoints={{ 768: { slidesPerView: 5 }, 1024: { slidesPerView: 6.5 } }} spaceBetween={12} autoplay={{ delay: 3500 }} navigation className="h-64">
             ...
             )) : <div className="text-white text-center p-10 font-black italic uppercase">Yükleniyor...</div>}
           </Swiper>
        </section>

        {/* KANKA: TEK SEKMELİ DEV SLIDER (Türkiye, Dünya, Asayiş, Siyaset eklendi) */}
        <section className="mt-4 bg-white border border-gray-200 shadow-lg overflow-hidden mb-8">
           <div className="flex bg-gray-900 overflow-x-auto no-scrollbar">
              {['TÜRKİYE', 'DÜNYA', 'ASAYİŞ', 'SİYASET'].map(tab => (
                 <button key={tab} onClick={() => setActiveKatTab(tab)} className={`flex-1 py-3 px-6 text-[12px] font-black italic uppercase transition-all duration-300 border-b-2 ${activeKatTab === tab ? 'bg-red-600 text-white border-red-800 shadow-inner' : 'text-gray-400 hover:bg-white/5 border-transparent'}`}>
                    {tab}
                 </button>
              ))}
           </div>
                 ...
                 {getKat(activeKatTab, 12).map(h => (
                    <SwiperSlide key={h.id}>
                       <Link href={`/haber/${h.id}`} className="relative block h-full group overflow-hidden">
                          <img src={h.resim} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-[8s]" />
                          <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent flex items-end justify-center p-10">
                             <h4 className="text-white text-2xl md:text-4xl font-black uppercase italic line-clamp-2 tracking-tighter text-center drop-shadow-2xl">{h.baslik}</h4>
                          </div>
                       </Link>
                    </SwiperSlide>
                 ))}
                 {getKat(activeKatTab, 12).length === 0 && (
                   <div className="h-full flex items-center justify-center font-black italic uppercase text-gray-400">Bu kategoride henüz haber mühürlenmedi kanka!</div>
                 )}
              </Swiper>
           </div>
        </section>
      ...
      <footer className="bg-[#0a0a0a] text-white mt-4 py-8 border-t-4 border-red-600 text-center uppercase font-black italic">
        ...
        <div className="flex justify-center flex-wrap gap-8 text-[10px] font-black text-gray-500 uppercase italic tracking-widest border-y border-white/5 py-4 mb-6">
           <Link href="/kunye" className="hover:text-red-600">KÜNYE</Link>
           <Link href="/iletisim" className="hover:text-red-600">İLETİŞİM</Link>
           <Link href="/reklam" className="hover:text-red-600">REKLAM</Link>
           <Link href="/kvkk" className="hover:text-red-600">KVKK</Link>
        </div>
